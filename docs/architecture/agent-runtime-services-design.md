...
Desktop 仍保留 `bitfun-core/product-full` 兼容 owner。CLI 与 CLI 托管的 ACP server 已消费各自的产品组装结果和
SDK 端口，但这不等于协调器、调度器、持久化或工具执行 owner 已迁移；ACP 的完整持久化历史、模型/模式、MCP、
客户端路径与 Desktop 入口仍保留明确的兼容边界。
...
`DeliveryProfile::Acp` 构造 `ProductRuntimeParts`。CLI 通过一个调用级上下文把
...
`interfaces/acp`。Desktop 尚未接入组装结果，Server 仅提供健康检查、信息与 ping 路由。未接入入口的 profile、
枚举分支和单元测试仍不能证明对应产品形态可用。
...
- 继续按真实复用需求缩小 ACP 的完整持久化历史、模型/模式、MCP 与客户端兼容路径；Desktop 入口另行接入产品组装并
  证明行为等价。ACP 生命周期和 Desktop 平台资源仍留在各自入口。
