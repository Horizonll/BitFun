...
| generated widget payload key | 245 | widget iframe 对外主题变量 allowlist，作为外部边界单独预算，不计入内部 alias 读取 |
...
| dark card white alpha literal | merge to overlay token | `ThemeService.ts` dark `--card-bg-*` runtime injection | `0.015/0.025/0.035/0.09/0.13` 仅是 dark card fallback 的微弱层级色，合并到现有 `white02/03/04/08/12` overlay token 后仍保留 default/elevated/subtle/accent/hover 语义 key |
...
| `generatedWidgetPayload.varUnique` | 245 | 245 | 控制 widget 对外主题 payload allowlist 不继续膨胀 |
