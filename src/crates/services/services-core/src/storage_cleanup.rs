...
impl CleanupService {
    pub fn new(roots: CleanupRoots, policy: CleanupPolicy) -> Self {
        Self { roots, policy }
    }

    pub async fn cleanup_all(&self) -> Result<CleanupResult, String> {
    ...
    async fn cleanup_temp_files(&self) -> Result<CleanupResult, String> {
        let retention = Duration::from_secs(self.policy.temp_retention_days * 24 * 3600);
        self.cleanup_old_files(&self.roots.temp_dir, retention)
            .await
    }

    async fn cleanup_old_logs(&self) -> Result<CleanupResult, String> {
        let retention = Duration::from_secs(self.policy.log_retention_days * 24 * 3600);
        self.cleanup_old_files(&self.roots.logs_dir, retention)
            .await
    }

    async fn cleanup_oversized_cache(&self) -> Result<CleanupResult, String> {
        let max_size = self.policy.max_cache_size_mb * 1_048_576;

        let current_size = Self::calculate_dir_size(&self.roots.cache_dir).await?;
        ...
        self.cleanup_by_size(&self.roots.cache_dir, max_size).await
    }
    ...
    async fn cleanup_by_size(&self, dir: &Path, max_size: u64) -> Result<CleanupResult, String> {
        let mut result = CleanupResult::default();
