...
2. 预设只接管「视觉身份」：配色、字体气质、装饰语言、版式偏好。本 skill 的核心原则全部继续生效——断言式标题、单页一结论、信息密度、反 AI slop、1280×720px 画布、可编辑 authoring contract、不许溢出。
...
3. **逐页 HTML**：封面 `slide-01`（标题/副标题/作者或日期）→ 按 outline 生成其余页；根据内容与风格选择可编辑图表、图示、文字或 intentional 内联图片，每页完整内联 CSS。**写每页时一次写对**：写之前先做垂直预算心算（见「防溢出预算」），写的时候同时遵守完整 authoring contract，写完即止。
