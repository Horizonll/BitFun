...
const MarkdownContent: React.FC<MarkdownContentProps> = ({ content, onFileDownload, onGetFileInfo }) => {
  ...
  }), [syntaxTheme, onFileDownload, onGetFileInfo]);
...
const TypewriterText: React.FC<{
  content: string;
  /** When false, show the full content immediately (turn completed). */
  animate?: boolean;
  onFileDownload?: (path: string, onProgress?: (downloaded: number, total: number) => void) => Promise<void>;
  onGetFileInfo?: (path: string) => Promise<{ name: string; size: number; mimeType: string }>;
}> = ({ content, animate = true, onFileDownload, onGetFileInfo }) => {
  const displayText = useTypewriter(content, animate);
...
const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // Stick-to-bottom while streaming. Important: never clear isNearBottom during
  // programmatic chase — a large content jump can leave gap >= threshold for a
  // moment and would otherwise permanently unpin follow (VR + typewriter growth).
  ...
  const scrollMessagesToBottom = useCallback((behavior: ScrollBehavior = 'auto') => {
    ...
    // Instant scrollTop is reliable in nested VR flex/WebView; avoid smooth chase lag.
  ...
  const handleScroll = useCallback(() => {
    ...
    if (programmaticScrollRef.current) {
      ...
      // Keep pinned while chasing; do not treat lag as user scroll-away.
    } else {
  ...
  const prevChatSessionRef = useRef<string | null>(null);
  useEffect(() => {
    initialScrollDone.current = false;
    pendingInitialScroll.current = false;
    // Only clear turn/hold on real session switch. Remounting the same session
    // (HUD ↔ focus) must keep a completed hold so the last tokens stay visible
    // until persisted messages arrive.
    if (prevChatSessionRef.current !== sessionId) {
      resetActiveTurnHold();
      setActiveTurn(null);
      prevChatSessionRef.current = sessionId;
    }
    ...
    Promise.all([loadMessages(), loadModelCatalog()]).then(([_, initialCatalog]) => {
      ...
      // Enable stick-to-bottom immediately; do not wait for a messages rerender.
      ...
      const poller = new SessionPoller(sessionMgr, sessionId, (resp: PollResponse) => {
        ...
        // Turn ended: desktop may clear active_turn / emit a truncated
        // assistant before the final tokens are persisted. Refresh with retries.
    ...
    return () => {
      ...
      pollerRef.current?.stop();
      pollerRef.current = null;
      // Keep activeTurn for HUD status when collapsing chat (VrShell poller owns it).
    };
  }, [
    appendNewMessages,
  ...
  }, [messages.length, isLoadingMore, scrollMessagesToBottom]);
  ...
  }, [optimisticMsg, scrollMessagesToBottom]);

  // Follow content height growth (stream tokens / typewriter), not only poll ticks.
  useEffect(() => {
    const container = messagesContainerRef.current;
  ...
  }, [inputExpanded, input, pendingImages.length, voiceListening]);
  ...
  return (
    ...
    <div className={`chat-page${layout === 'vr' ? ' chat-page--vr' : ''}${voiceOnlyComposer ? ' chat-page--voice-only' : ''}${glassesUiActive ? ' chat-page--glasses-ui' : ''}`}>
      ...
      <div className="chat-page__messages" ref={messagesContainerRef} onScroll={handleScroll}>
        <div className="chat-page__messages-inner" ref={messagesContentRef}>
        {isLoadingMore && (
        ...
        {(() => {
          ...
          return messages.map((m, idx) => {
            ...
            // While the turn overlay (stream / completed hold) is visible, hide
            // the in-flight assistant row so a truncated persist does not flash.
            if (activeTurn && m.role === 'assistant' && idx > lastUserIdx) {
              return null;
            }
            ...
            <div className="chat-page__input-actions-right">
              ...
              {(layout === 'vr' || enableVoice) && (
                <button
                  ...
                  title={voiceListening ? t('chat.voiceListening') : t('chat.voiceInput')}
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
