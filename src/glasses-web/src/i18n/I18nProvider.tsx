...
// Glasses product locale store (do not inherit mobile-web English preference).
const STORAGE_KEY = 'bitfun-glasses-language';
const GLASSES_DEFAULT_LANGUAGE: MobileLanguage = 'zh-CN';
...
function detectInitialLanguage(): MobileLanguage {
  ...
  // Optical glasses companion always defaults to Simplified Chinese.
  return GLASSES_DEFAULT_LANGUAGE;
}
...
export const I18nContext = createContext<I18nContextValue>({
  language: GLASSES_DEFAULT_LANGUAGE,
  setLanguage: () => {},
  toggleLanguage: () => {},
  t: (key) => key,
  formatDate: (date, options) => new Intl.DateTimeFormat(GLASSES_DEFAULT_LANGUAGE, options).format(date),
  formatRelativeTime: (date) => formatRelativeTime(GLASSES_DEFAULT_LANGUAGE, date),
});
